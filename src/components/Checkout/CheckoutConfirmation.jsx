...
const CheckoutConfirmation = () => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto px-4 py-6 space-y-6 overflow-y-auto">
      {/* Success Animation */}
      <div className="flex flex-col items-center text-center">
        <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-green-100 flex items-center justify-center mb-4 animate-pulse">
          <CheckCircle className="w-10 h-10 sm:w-12 sm:h-12 text-green-500" />
        </div>

        {/* Success Message */}
        <h2 className="text-xl sm:text-2xl font-bold mb-2 text-gray-900">Payment Successful! 🎉</h2>
      ...
      <div className="bg-gray-50 rounded-lg p-4 sm:p-6">
        <div className="flex items-center justify-between mb-4">
          <span className="text-xs sm:text-sm text-gray-500">Order Number</span>
          <span className="font-mono text-xs sm:text-sm font-medium break-all">{orderNumber}</span>
        </div>
        ...
        <div className="space-y-2 mb-4">
          {items.map(item => (
            <div key={`${item.product.id}-${item.selectedColor}-${item.selectedSize}`} 
              ...
              <span className="font-medium flex-shrink-0">₹{item.product.price * item.quantity}</span>
            </div>
          ))}
        </div>
      ...
      <div className="space-y-4">
        <h3 className="font-semibold text-center mb-4 text-sm sm:text-base">Order Timeline</h3>
        ...
        <div className="space-y-4">
          <div className="flex items-start space-x-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="font-medium text-xs sm:text-sm">Payment Confirmed</div>
          ...
          <div className="flex items-start space-x-3">
            <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-yellow-100 flex items-center justify-center flex-shrink-0">
              <Package size={14} className="sm:w-4 sm:h-4 text-yellow-600" />
            </div>
            <div className="min-w-0 flex-1">
              <div className="font-medium text-xs sm:text-sm">Order Processing</div>
          ...
          <div className="flex items-start space-x-3">
            <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0">
              <Truck size={14} className="sm:w-4 sm:h-4 text-blue-600" />
            </div>
            <div className="min-w-0 flex-1">
              <div className="font-medium text-xs sm:text-sm">Shipped</div>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="font-medium text-xs sm:text-sm text-gray-500">Delivered</div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <button 
          ...
          className="flex-1 bg-black text-white py-3 rounded-lg font-medium hover:bg-gray-800 transition-colors duration-200 text-sm sm:text-base"
        >
          Track Order
        </button>
        <button 
          onClick={() => navigate('/')}
          className="flex-1 border border-gray-300 text-gray-700 py-3 rounded-lg font-medium hover:bg-gray-50 transition-colors duration-200 text-sm sm:text-base"
        >
          Continue Shopping
        </button>
      </div>
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
        <div className="flex items-start space-x-2">
          ...
          <div className="min-w-0 flex-1">
            <div className="font-medium text-yellow-900 text-xs sm:text-sm">Demo Mode</div>
